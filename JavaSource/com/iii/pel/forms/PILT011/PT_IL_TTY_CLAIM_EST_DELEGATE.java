	...
	public void executeSelectStatement(
		...
		try {
			...
			if(cover_count>1 ){
				...
				selectStatement ="SELECT RCS_CLAIM_COVER_CODE, RCA_TREATY_CODE, RCA_TREATY_TYPE  \"Treaty Type\", RCA_RET_FC_CLM_EST \"Retention\", RCA_RI_FC_CLM_EST \"RI Recovery\" FROM PT_IL_RI_CLAIM_ALLOC WHERE RCA_CLM_SYS_ID = ? ";
				...
				selectStatement ="SELECT RCA_CLAIM_COVER_CODE, RCA_TREATY_CODE, RCA_TREATY_TYPE  \"Treaty Type\", RCA_RET_FC_CLM_EST \"Retention\", RCA_RI_FC_CLM_EST \"RI Recovery\" FROM PT_IL_RI_CLAIM_ALLOC WHERE RCA_CLM_SYS_ID = ?  AND RCA_CLAIM_COVER_CODE = ?";
				 resultSet = new CRUDHandler().executeSelectStatement(
			...
			while (resultSet.next()) {
				...
				PT_IL_TTY_CLAIM_EST_BEAN.setTCE_CCD_COVER_CODE(resultSet.getString("RCA_CLAIM_COVER_CODE"));
				PT_IL_TTY_CLAIM_EST_BEAN.setTCE_TREATY_CODE(resultSet.getString("RCA_TREATY_CODE"));
				PT_IL_TTY_CLAIM_EST_BEAN.setTCE_RETN_LC_CLM_EST(resultSet.getDouble("Retention"));
				PT_IL_TTY_CLAIM_EST_BEAN.setTCE_QS_LC_CLM_EST(resultSet.getDouble("RI Recovery"));
				PT_IL_TTY_CLAIM_EST_BEAN.setTCE_CLAIM_LOSS_DT_FT(resultSet.getString("Treaty Type"));
