...
public interface PILR244_012_QUERY_CONSTANTS {
	...
	String USER_ID_QUERY = "SELECT USER_ID, USER_DESC FROM MENU_USER WHERE MENU_USER.USER_FRZ_FLAG = 'N' AND ROWNUM<=? ORDER BY 1";

	String USER_ID_QUERY_LIKE = " SELECT USER_ID, USER_DESC FROM MENU_USER WHERE MENU_USER.USER_FRZ_FLAG = 'N' AND (USER_ID LIKE ? OR USER_DESC LIKE ?)"
			+ " AND ROWNUM<=? ORDER BY 1";

	String BANK_CODE_QUERY = " SELECT CLASS_CODE,CLASS_NAME FROM PM_IL_CLASS WHERE PM_IL_CLASS.CLASS_FRZ_FLAG = 'N' "
			+ " AND ROWNUM <=? ORDER BY 1";

	String BANK_CODE_QUERY_LIKE = " SELECT CLASS_CODE,CLASS_NAME FROM PM_IL_CLASS WHERE PM_IL_CLASS.CLASS_FRZ_FLAG = 'N' "
			+ " AND (CLASS_CODE LIKE ? OR CLASS_NAME LIKE ?) AND ROWNUM <=? ORDER BY 1";
	...
	String AGENT_QUERY = " SELECT CUST_CODE, CUST_NAME FROM PM_CUSTOMER,PM_CUST_CLASS "
			+ " WHERE CCLAS_TYPE IN (?,?,?,?,?,?,?,?,?,?) AND CUST_CLASS = CCLAS_CODE  AND ROWNUM<=? ORDER BY 1 ";

	String AGENT_QUERY_LIKE = " SELECT CUST_CODE, CUST_NAME FROM PM_CUSTOMER,PM_CUST_CLASS WHERE CCLAS_TYPE IN (?,?,?,?,?,?,?,?,?,?) AND CUST_CLASS = CCLAS_CODE"
