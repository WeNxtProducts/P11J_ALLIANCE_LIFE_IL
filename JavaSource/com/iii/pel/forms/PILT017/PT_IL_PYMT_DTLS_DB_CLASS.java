...
public class PT_IL_PYMT_DTLS_DB_CLASS {
	...
	public ArrayList<PT_IL_PYMT_DTLS> fetchValues(String paymentTransactionType,Long paymentTransactionSysid){
		...
		try {
			...
			if(con!= null){
				...
				while(rst.next()){
					...
					/*Commented and modified by piduguraj dt: 27-09-2020 for bank name to show in onload
					 * listBean.setPD_BANK_NAME(rst.getString("PD_BANK_NAME"));
							bankDesc = getBankDesc(rst.getString("PD_BANK_NAME"));*/
					listBean.setPD_BANK_NAME(rst.getString("PD_ADDR_02"));
					bankDesc = getBankDesc(rst.getString("PD_ADDR_02"));
					...
					if(rst.getDate("PD_CC_EXP_DT") != null){
				    	/*Newly added by pidugu raj dt: 04-10-2018 for expiry date as suggested by vaitheeswaran*/
						String expDt = (rst.getDate("PD_CC_EXP_DT").toString());
						listBean.setPOL_CARD_EXP_DT_MM(expDt.substring(5,7));
						listBean.setPOL_CARD_EXP_DT_YY(expDt.substring(2,4));
						/*Newly added by pidugu raj dt: 04-10-2018 for expiry date as suggested by vaitheeswaran*/
				    }
					/*End Newly added by pidugu raj for kic dt: 04-10-2018 as suggested by sivaraman*/
					...
					listBean.setPD_MICR_CODE(rst.getString("PD_MICR_CODE"));
					

					/*Added by Janani for Fidelity on 09.06.2018 as suggested by Sivaram */
				
					listBean.setPD_EXCH_RATE(rst.getDouble("PD_EXCH_RATE"));

					/*End*/
					
					
					/*Newly added by pidugu raj for kic dt: 04-10-2018 as suggested by sivaraman*/
					listBean.setPD_CVV_NO(rst.getInt("PD_CVV_NO"));
	...
	public List currCodeLov(String currCode){
		...
		try 
		{
			...
			else
			{
				...
				query = "SELECT CURR_CODE, CURR_NAME FROM FM_CURRENCY WHERE upper(CURR_CODE) LIKE UPPER('"+currCode+"%')";
				/*end*/
			}
	...
	public String saveData(PT_IL_PYMT_DTLS_ACTION pymtDtlAction)
	{
		Connection connection = null;
		CRUDHandler handler = new CRUDHandler();
		...
		try {
			...
			Long sysId=pymtDtlAction.pilt017_pt_il_pymt_dtls_pre_insert();
	...
	public String getProposalLevelPaymentType(Long paymentTransactionSysid){
		String pol_Pymt_Type_Query = PILT017_QUERY_CONSTANTS.PILT017_FETCH_POL_YMT_TYPE_QUERY;
		ResultSet rst = null;
		CRUDHandler handler = null;
		Connection con = null;
		String pol_Pymt_Type = null;
		FacesContext context = FacesContext.getCurrentInstance();
		try {
			con = CommonUtils.getConnection();
			if(con!= null){
				handler = new CRUDHandler();
				Object[] values={ paymentTransactionSysid };
				rst = handler.executeSelectStatement(pol_Pymt_Type_Query, con, values);
				if(rst.next()){
					pol_Pymt_Type = rst.getString("POL_PYMT_TYPE");
				}
			}
		} catch (DBException e) {
			e.printStackTrace();
