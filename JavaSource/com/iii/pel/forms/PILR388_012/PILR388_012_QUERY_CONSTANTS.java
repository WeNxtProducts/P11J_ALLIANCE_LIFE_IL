...
public interface PILR388_012_QUERY_CONSTANTS {
	...
	String AGENT_CODE_QUERY = "SELECT CUST_CODE,CUST_NAME ,CUST_FRZ_FLAG FROM PM_CUSTOMER WHERE  CUST_CLASS  IN (SELECT CUST_CLASS FROM   PM_CUST_CLASS WHERE CCLAS_TYPE = '002') AND CUST_FRZ_FLAG ='N' AND ROWNUM<=? ORDER BY 1";
	String AGENT_CODE_QUERY_LIKE = "SELECT CUST_CODE,CUST_NAME ,CUST_FRZ_FLAG FROM PM_CUSTOMER WHERE  CUST_CLASS  IN (SELECT CUST_CLASS FROM   PM_CUST_CLASS WHERE CCLAS_TYPE = '002') AND CUST_FRZ_FLAG ='N' AND (CUST_CODE LIKE ? OR CUST_NAME LIKE ?) AND ROWNUM<=? ORDER BY 1";
	String UNIT_CODE_QUERY ="SELECT PC_CODE,PC_DESC FROM PM_CODES WHERE PC_TYPE ='UNITCODE' AND PC_FRZ_FLAG='N' AND ROWNUM<=? ORDER BY 1";
	String UNIT_CODE_QUERY_LIKE ="SELECT PC_CODE,PC_DESC FROM PM_CODES WHERE PC_TYPE ='UNITCODE' AND PC_FRZ_FLAG='N' AND (PC_CODE LIKE ? OR PC_DESC LIKE ? ) AND ROWNUM<=? ORDER BY 1";
