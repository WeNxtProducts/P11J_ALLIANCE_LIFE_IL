...
public class PM_IL_DOC_TODO_GROUP_ACTION extends CommonAction {
	...
	private PT_IL_OCD_DOC_TODO_LIST_HELPER helperDocToDoList;
	
	private PT_IL_OCD_DOC_TODO_LIST_ACTION todolistAction;
	...
	public PM_IL_DOC_TODO_GROUP_ACTION() {
		...
		helperDocToDoList = new PT_IL_OCD_DOC_TODO_LIST_HELPER();
		todolistAction=new PT_IL_OCD_DOC_TODO_LIST_ACTION();
	...
	private void resetAllComponent() {
		...
		compositeAction.getPT_IL_OCD_DOC_TODO_LIST_ACTION_BEAN().getCOMP_DTLS_APPR_STS().resetValue();
		compositeAction.getPT_IL_OCD_DOC_TODO_LIST_ACTION_BEAN().getCOMP_DTLS_MANDATORY_YN().resetValue();
		//compositeAction.getPT_IL_DOC_TODO_LIST_STATUS_ACTION_BEAN().COMP_DTLS_APPR_STS.resetValue();
		compositeAction.getPT_IL_OCD_DOC_TODO_LIST_ACTION_BEAN().getCOMP_DTLS_DOC_REC_DT().resetValue();
		compositeAction.getPT_IL_OCD_DOC_TODO_LIST_ACTION_BEAN().getCOMP_DTLS_REMARKS().resetValue();
	}
	...
	public void onLoad(PhaseEvent event) {
		try {
			Long polsysId = (Long) CommonUtils.getGlobalObject("OCD_POL_SYS_ID");
			Long transsysId = (Long) CommonUtils.getGlobalObject("OCD_TRANS_SYS_ID");
			System.out.println("polsysId  "+polsysId);
			System.out.println("transsysId  "+transsysId);
			if (isBlockFlag()) {
				...
				helper.preQuery(PM_IL_DOC_TODO_GROUP_BEAN);
					...
					helper.executeQuery1(compositeAction);
					if("A".equalsIgnoreCase(compositeAction.getPT_IL_PAYOUT_PROCESS_ACTION_BEAN().getPT_IL_PAYOUT_PROCESS_BEAN().getPP_PAYOUT_STATUS()))
					{
				...
				compositeAction.getPT_IL_OCD_DOC_TODO_LIST_ACTION_BEAN().getPT_IL_OCD_DOC_TODO_LIST_BEAN().setDataTable(dataTable);
			
				
			}
		/*	if("D".equalsIgnoreCase(compositeAction.getPT_IL_DOC_TODO_LIST_STATUS_ACTION_BEAN().getPT_IL_DOC_TODO_LIST_STATUS_BEAN().getDTLS_APPR_STS())){
				compositeAction.getPT_IL_DOC_TODO_LIST_STATUS_ACTION_BEAN().getCOMP_DTLS_DOC_REC_DT_LABEL().setValue(Messages.getString(CommonUtils.getGlobalVariable("GLOBAL.SITE_CODE")+"messageProperties_PILT002_APAC",
						"PILT002_APAC$PT_IL_DOC_TODO_LIST_STATUS$DTLS_DOC_DEF_TILL$caption"));
			}else{
				compositeAction.getPT_IL_DOC_TODO_LIST_STATUS_ACTION_BEAN().getCOMP_DTLS_DOC_REC_DT_LABEL().setValue(Messages.getString(CommonUtils.getGlobalVariable("GLOBAL.SITE_CODE")+"messageProperties_PILT002_APAC",
						"PILT002_APAC$PT_IL_DOC_TODO_LIST_STATUS$DTLS_DOC_REC_DT$caption"));
			}*/
		} catch (Exception e) {
			e.printStackTrace();
			getErrorMap().put(PELConstants.PEL_ERROR_WARNING_CURRENT_KEY,
