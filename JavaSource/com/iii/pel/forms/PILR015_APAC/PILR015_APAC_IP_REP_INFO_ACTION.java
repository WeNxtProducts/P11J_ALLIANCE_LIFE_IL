package com.iii.pel.forms.PILR015_APAC;
...
public class PILR015_APAC_IP_REP_INFO_ACTION  extends CommonReportAction{
	...
	PILR015_APAC_HELPER helper;
	PILR015_APAC_DELEGATE delegate;
	
	private List<SelectItem> listM_MODULE_CODE_FM;
	
	public PILR015_APAC_IP_REP_INFO_ACTION(){
		
		IP_REP_INFO_BEAN=new IP_REP_INFO();
		rerenderComponents();
		setComboBoxValues();
		helper=new PILR015_APAC_HELPER();
		delegate=new PILR015_APAC_DELEGATE();
	...
	public void prepareDropDownList() {
		...
		try {
			...
			listM_MODULE_CODE_FM = ListItemUtil.getDropDownListValue(connection, "PILR015_APAC", 
...
//				new PGLR039_012_DELEGATE().executeSaveAction(this.IP_REP_INFO_BEAN);
				response = (HttpServletResponse) FacesContext
						...
						.getResponse();
				map.put("P_FM_SRC_BUS_CLASS",IP_REP_INFO_BEAN.getUI_M_FM_SRC_BUS_CLASS());
				map.put("P_TO_SRC_BUS_CLASS",IP_REP_INFO_BEAN.getUI_M_TO_SRC_BUS_CLASS());
				map.put("P_FM_POL_PROD_CODE",IP_REP_INFO_BEAN.getUI_M_FM_PROD_CODE());
				map.put("P_TO_POL_PROD_CODE",IP_REP_INFO_BEAN.getUI_M_TO_PROD_CODE());
				map.put("P_MODULE_CODE_FM",IP_REP_INFO_BEAN.getUI_M_MODULE_CODE_FM());
				map.put("P_MODULE_CODE_TO",IP_REP_INFO_BEAN.getUI_M_MODULE_CODE_TO());
...
//				response.sendRedirect(redirectUrl);
	 ...
	 public void onLoad(PhaseEvent phaseEvent){
		 ...
		 PILR015_APAC_HELPER helper=new PILR015_APAC_HELPER();
...
//			 IP_REP_INFO_BEAN.setSelectedCustStatus(getCustStatusList().get(0).getValue().toString());
	...
	public void whenValidateUI_M_FM_SRC_BUS_CLASS(ActionEvent actionEvent) {
		cancelModalPanel();
		CommonUtils.clearMaps(this);
		UIInput input = (UIInput) actionEvent.getComponent().getParent();
		
		if(input.getSubmittedValue()==null || "".equals(input.getSubmittedValue())){
			this.IP_REP_INFO_BEAN.setUI_M_FM_SRC_BUS_CLASS(REPORT_UTIL_CONSTANTS.SET_FIRST_FIELD_DEFAULT_0);
			getIP_REP_INFO_BEAN().setUI_M_FM_SRC_BUS_NAME(null);
		}else{
			ErrorHelpUtil.validate(input, getErrorMap());
			if (input.isValid()) {
				this.IP_REP_INFO_BEAN.setUI_M_FM_SRC_BUS_CLASS(input.getValue().toString());
			}
		}
	...
	public void validateUI_M_FM_SRC_BUS_CLASS(FacesContext facesContext,
		...
		try {
			if(!"*".equals(uiDivnFmCode)){
				...
				IP_REP_INFO_BEAN.setUI_M_FM_SRC_BUS_NAME(divnCodeName);
			}
			COMP_1_INPUT_TEXT_DESC.resetValue();
	...
	public List<LovBean> lovUI_M_DIVISION(Object obj){
		...
		if("*".equals(code)){
			...
			query=PILR015_APAC_QUERY_CONSTANTS.DIVISION_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR015_APAC_QUERY_CONSTANTS.DIVISION_QUERY_LIKE;
		}
		suggestionList = delegate.prepareLovList(query, object);
		return suggestionList;
	}
	
	public void whenValidateUI_M_TO_SRC_BUS_CLASS(ActionEvent actionEvent) {
		cancelModalPanel();

		CommonUtils.clearMaps(this);
		UIInput input = (UIInput) actionEvent.getComponent().getParent();
		if(input.getSubmittedValue()==null || input.getSubmittedValue().toString().isEmpty()){
			if(IP_REP_INFO_BEAN.getUI_M_FM_SRC_BUS_CLASS()!=null){
				if(REPORT_UTIL_CONSTANTS.SET_FIRST_FIELD_DEFAULT_0.equals(IP_REP_INFO_BEAN.getUI_M_FM_SRC_BUS_CLASS())){
					this.IP_REP_INFO_BEAN.setUI_M_TO_SRC_BUS_CLASS(REPORT_UTIL_CONSTANTS.SET_SECOND_FIELD_DEFAULT_ZZZZ);
					getIP_REP_INFO_BEAN().setUI_M_TO_SRC_BUS_NAME(null);
				}else{
					this.IP_REP_INFO_BEAN.setUI_M_TO_SRC_BUS_CLASS(IP_REP_INFO_BEAN.getUI_M_FM_SRC_BUS_CLASS());
					this.IP_REP_INFO_BEAN.setUI_M_TO_SRC_BUS_NAME(IP_REP_INFO_BEAN.getUI_M_FM_SRC_BUS_NAME());
				}
			}
			...
			ErrorHelpUtil.validate(input, getErrorMap());
			if (input.isValid()) {
				this.IP_REP_INFO_BEAN.setUI_M_TO_SRC_BUS_CLASS(input.getValue().toString());
			}
		}
	...
	public void validateUI_M_TO_SRC_BUS_CLASS(FacesContext facesContext,
			UIComponent component, Object value) throws ValidatorException {

		String uiDivnToCode = (String) value;
		try {
			String divnCodeName = helper.WHEN_VALIDATE_ITEM_UI_DIVN_CODE(uiDivnToCode,IP_REP_INFO_BEAN);
			IP_REP_INFO_BEAN.setUI_M_TO_SRC_BUS_NAME(divnCodeName);
			COMP_2_INPUT_TEXT_DESC.resetValue();
	...
	public List<LovBean> lovSourceOfBus(Object value){
		ListItemUtil listItemUtil = null;
		ArrayList<LovBean> list = new ArrayList<LovBean>();
		try {
			listItemUtil = new ListItemUtil();
			list = listItemUtil.P_CALL_LOV("PILR015_APAC", "REP_PARAM", "M_FM_SRC_BUS_CLASS", 
	...
	public List<LovBean> lovProdCode(Object value){
		...
		try {
			listItemUtil = new ListItemUtil();
			list = listItemUtil.P_CALL_LOV("PILR015_APAC", "REP_PARAM", "M_TO_POL_PROD_CODE", 
	...
	public List<LovBean> lovUI_M_CUST_CLASS(Object obj){
		...
		String query=null;
		 if("*".equals(code)){
			object=new Object[]{PELConstants.suggetionRecordSize};
			query=PILR015_APAC_QUERY_CONSTANTS.CUST_CLASS_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR015_APAC_QUERY_CONSTANTS.CUST_CLASS_QUERY_LIKE;
		}
