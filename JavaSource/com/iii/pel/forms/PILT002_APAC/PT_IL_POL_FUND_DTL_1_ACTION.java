...
public class PT_IL_POL_FUND_DTL_1_ACTION extends CommonAction {
	...
	public void postRecord(ActionEvent event) {
		String message = null;
		try {
			if (PT_IL_POL_FUND_DTL_1_BEAN.getROWID() == null && isINSERT_ALLOWED()) {
				helper.PRE_INSERT(compositeAction);
				new CRUDHandler().executeInsert(PT_IL_POL_FUND_DTL_1_BEAN,CommonUtils.getConnection());
			...
			} else if (PT_IL_POL_FUND_DTL_1_BEAN.getROWID() != null && isUPDATE_ALLOWED()) {
				helper.PRE_UPDATE(compositeAction);
				new CRUDHandler().executeUpdate(PT_IL_POL_FUND_DTL_1_BEAN,CommonUtils.getConnection());
			...
			e.printStackTrace();
			getErrorMap().put(PELConstants.PEL_ERROR_WARNING_CURRENT_KEY,"Record not Inserted/Updated :: " + e.getMessage());
			getErrorMap().put("postRecord","Record not Inserted/Updated :: " + e.getMessage());
		}
	}
	...
	public void displayRecords() {
		...
		try {
			resetSelectedRow();
			PT_IL_POL_FUND_DTL_1_BEAN = (PT_IL_POL_FUND_DTL_1) dataTable.getRowData();
			helper.POST_QUERY(PT_IL_POL_FUND_DTL_1_BEAN);
	...
	public void onLoad(PhaseEvent event) {
		try {
			...
			if (isBlockFlag()) {
				...
				helper.WHEN_NEW_RECORD_INSTANCE(this);
				setBlockFlag(false);
				new PT_IL_POLICY_ACTION().enableDisableForApproveCondition();
			}
