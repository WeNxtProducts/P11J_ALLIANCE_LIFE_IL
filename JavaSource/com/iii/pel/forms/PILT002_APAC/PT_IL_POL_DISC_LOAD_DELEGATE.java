	...
	public void executeSelectStatement(
		...
		try {
			...
			while (resultSet.next()) {
				...
				//addded by saranya
				System.out.println("pdlemrrate-->["+resultSet.getDouble("PDL_EMR_RATE")+"]");
				PT_IL_POL_DISC_LOAD_BEAN.setPDL_EMR_RATE((resultSet.getDouble("PDL_EMR_RATE")>0)?(resultSet.getDouble("PDL_EMR_RATE")):(resultSet.getDouble("PDL_RATE")));
				//end
				
				/* Added by rakesh on 02-08-2019 for disc/loading applied on*/
				PT_IL_POL_DISC_LOAD_BEAN.setPDL_LOAD_APPL_ON(resultSet
						.getString("PDL_LOAD_APPL_ON"));
				//END
