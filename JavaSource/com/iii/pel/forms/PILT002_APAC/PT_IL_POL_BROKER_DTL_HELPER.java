...
public class PT_IL_POL_BROKER_DTL_HELPER {
	...
	public void preUpdate(PT_IL_POL_BROKER_HEAD brokerHeadBean,
		...
		String CURSOR_C1 = "SELECT POBD_YEAR_FROM, POBD_YEAR_TO FROM PT_IL_POL_BROKER_DTL WHERE POBD_POBH_SYS_ID  =  ? AND  ROWID  != ?";
		Double M_POBD_YEAR_FROM = null;
		Double M_POBD_YEAR_TO = null;
		ResultSet resultSet = null;
		Connection connection = null;
		...
		try {
			...
			resultSet = handler.executeSelectStatement(CURSOR_C1, connection,
					new Object[] { brokerHeadBean.getPOBH_SYS_ID(),brokerDetailBean.getROWID()});
			while (resultSet.next()) {
				M_POBD_YEAR_FROM = resultSet.getDouble(1);
				M_POBD_YEAR_TO = resultSet.getDouble(2);
				if (brokerDetailBean.getPOBD_YEAR_FROM() >= M_POBD_YEAR_FROM
	...
	public void whenValidateRecord(PT_IL_POL_BROKER_DTL brokerDetailBean,PILT002_APAC_COMPOSITE_ACTION compositeAction)
			throws Exception {
...
PT_IL_POL_BROKER_HEAD PT_IL_POL_BROKER_HEAD_BEAN = compositeAction.getPT_IL_POL_BROKER_HEAD_ACTION_BEAN().getPT_IL_POL_BROKER_HEAD_BEAN();
			...
			if (brokerDetailBean.getROWID() == null) {
				resultSet = handler.executeSelectStatement(CURSOR_C1,
						connection, new Object[] {
								...
								brokerDetailBean.getPOBD_YEAR_FROM(),
								brokerDetailBean.getPOBD_YEAR_TO(),PT_IL_POL_BROKER_HEAD_BEAN.getPOBH_SYS_ID()
	...
	public String checkModifyRate(PILT002_APAC_COMPOSITE_ACTION compositeAction) throws Exception{
      ResultSet resultSet = null;
      CRUDHandler handler =  new CRUDHandler();
      Connection connection = null;
      String asd_char_value = null;
	  String C1 = " SELECT  ASD_CHAR_VALUE FROM    PM_IL_APPR_SETUP_DET WHERE   ASD_USER_ID = ? "
			+ "   AND     ASD_CODE    = 'MOD_COMM_YN' "
	  ...
	  try {
		connection = CommonUtils.getConnection();
		  ...
		  if(resultSet.next()){
			  asd_char_value = resultSet.getString(1);
		  }
	} catch (DBException e) {
		e.printStackTrace();
		throw new Exception(e.getMessage());
