...
public class PT_IL_POL_FUND_DTL_DELEGATE {
    public void executeSelectStatement(PILT002_APAC_COMPOSITE_ACTION compositeAction)throws Exception {
	
   String selectStatement = "SELECT ROWID,PT_IL_POL_FUND_DTL.* FROM PT_IL_POL_FUND_DTL  " +
   		" WHERE PT_IL_POL_FUND_DTL.POFD_POL_SYS_ID = ?  AND PT_IL_POL_FUND_DTL.POFD_FUND_DIR_TYPE = 'P' ";
   ...
   try {
		...
		compositeAction.getPT_IL_POL_FUND_DTL_ACTION_BEAN().setPT_IL_POL_FUND_DTL_BEAN(PT_IL_POL_FUND_DTL_BEAN);
		new PT_IL_POL_FUND_DTL_HELPER().POST_QUERY(compositeAction);
	    }
	}
   catch (Exception e) {
		e.printStackTrace();
