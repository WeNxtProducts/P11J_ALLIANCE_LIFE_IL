...
public class AGENT_PROCESS_HELPER {
	...
	public ArrayList<String> PR_AGENT_OTHER_BENEFITS(AGENT_PROCESS aGENT_PROCESS_BEAN) throws SQLException, Exception{
		Connection connection = null;
		connection = CommonUtils.getConnection();
		String P_AGT_FM = aGENT_PROCESS_BEAN.getUI_M_AGT_FM();
		String P_AGT_TO = aGENT_PROCESS_BEAN.getUI_M_AGT_TO();
		
		String P_PROD_FM_DT = CommonUtils.dateToStringFormatter(aGENT_PROCESS_BEAN.getUI_M_PROD_FM_DT());
		String P_PROD_TO_DT = CommonUtils.dateToStringFormatter(aGENT_PROCESS_BEAN.getUI_M_PROD_TO_DT());
		String P_PROCESS_DT = CommonUtils.dateToStringFormatter(aGENT_PROCESS_BEAN.getUI_M_PROCESS_DT());
		ArrayList<OracleParameter> parameterList = new ArrayList<OracleParameter>();
		ArrayList<OracleParameter> outputList = new ArrayList<OracleParameter>();
		ArrayList<String> returnList = new ArrayList<String>();
		OracleParameter param1 = new OracleParameter("IN1", "STRING", "IN",P_AGT_FM);
		...
		OracleParameter param3 = new OracleParameter("IN5", "STRING", "IN",P_PROD_FM_DT);
		parameterList.add(param3);
		OracleParameter param4 = new OracleParameter("IN6", "STRING", "IN",P_PROD_TO_DT);
		parameterList.add(param4);
		OracleParameter param5 = new OracleParameter("IN7", "STRING", "IN",P_PROCESS_DT);
		parameterList.add(param5);

		OracleProcedureHandler procHandler = new OracleProcedureHandler();
		outputList = procHandler.execute(parameterList, connection,
				"P9ILPK_AGENT_BENEFITS.PR_AGENT_OTHER_BENEFITS");
		Iterator<OracleParameter> iterator = outputList.iterator();
		while (iterator.hasNext()) {
			OracleParameter oracleParameter = iterator.next();
			System.out.println("Output value:::" + oracleParameter.getValue());
