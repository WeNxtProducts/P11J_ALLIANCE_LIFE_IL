...
public class PILR_LOAN_SCHD_IP_REP_INFO_ACTION  extends CommonReportAction{
	...
	PILR_LOAN_SCHD_HELPER helper;
	PILR_LOAN_SCHD_DELEGATE delegate;
	
	private List<SelectItem> listM_MODULE_CODE_FM;
	
	public PILR_LOAN_SCHD_IP_REP_INFO_ACTION(){
		
		IP_REP_INFO_BEAN=new IP_REP_INFO();
		rerenderComponents();
		setComboBoxValues();
		helper=new PILR_LOAN_SCHD_HELPER();
		delegate=new PILR_LOAN_SCHD_DELEGATE();
	...
	public String runAction() {
		...
		try
		{
			if (IP_REP_INFO_BEAN.getSelectedFileType() != null) {
				...
				map.put("P_LANG_CODE",IP_REP_INFO_BEAN.getREP_VALUE_21());
				map.put("REP_KEY_NO",IP_REP_INFO_BEAN.getREP_KEY_NO());
...
//				response.sendRedirect(redirectUrl);
	 ...
	 public void onLoad(PhaseEvent phaseEvent){
		 ...
		 PILR_LOAN_SCHD_HELPER helper=new PILR_LOAN_SCHD_HELPER();
...
//			 IP_REP_INFO_BEAN.setSelectedCustStatus(getCustStatusList().get(0).getValue().toString());
	...
	public List<LovBean> lovUI_M_DIVISION(Object obj){
		...
		if("*".equals(code)){
			...
			query=PILR_LOAN_SCHD_QUERY_CONSTANTS.DIVISION_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR_LOAN_SCHD_QUERY_CONSTANTS.DIVISION_QUERY_LIKE;
		}
	...
	public List<LovBean> lovClientCode(Object value){
	...
	public List<LovBean> lovUI_M_CUST_CLASS(Object obj){
		...
		String query=null;
		 if("*".equals(code)){
			object=new Object[]{PELConstants.suggetionRecordSize};
			query=PILR_LOAN_SCHD_QUERY_CONSTANTS.CUST_CLASS_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR_LOAN_SCHD_QUERY_CONSTANTS.CUST_CLASS_QUERY_LIKE;
		}
