...
public class PILR307_012_IP_REP_INFO_ACTION  extends CommonReportAction{
	...
	PILR307_012_HELPER helper;
	PILR307_012_DELEGATE delegate;
	
	private List<SelectItem> listM_MODULE_CODE_FM;
	
	public PILR307_012_IP_REP_INFO_ACTION(){
		
		IP_REP_INFO_BEAN=new IP_REP_INFO();
		rerenderComponents();
		setComboBoxValues();
		helper=new PILR307_012_HELPER();
		delegate=new PILR307_012_DELEGATE();
...
//				new PGLR039_012_DELEGATE().executeSaveAction(this.IP_REP_INFO_BEAN);
				...
				map.put("P_FM_POL_NO",IP_REP_INFO_BEAN.getUI_M_FM_POL_NO());
				map.put("P_TO_POL_NO",IP_REP_INFO_BEAN.getUI_M_TO_POL_NO());
				map.put("P_FM_PROD_CODE",IP_REP_INFO_BEAN.getUI_M_FM_PROD_CODE());
				map.put("P_TO_PROD_CODE",IP_REP_INFO_BEAN.getUI_M_TO_PROD_CODE());
				map.put("P_FM_TXN_DT",IP_REP_INFO_BEAN.getREP_VALUE_7());
				map.put("P_TO_TXN_DT",IP_REP_INFO_BEAN.getREP_VALUE_8());
...
//				response.sendRedirect(redirectUrl);
	 ...
	 public void onLoad(PhaseEvent phaseEvent){
		 ...
		 PILR307_012_HELPER helper=new PILR307_012_HELPER();
...
//			 IP_REP_INFO_BEAN.setSelectedCustStatus(getCustStatusList().get(0).getValue().toString());
	...
	public List<LovBean> lovUI_M_DIVISION(Object obj){
		...
		if("*".equals(code)){
			...
			query=PILR307_012_QUERY_CONSTANTS.DIVISION_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR307_012_QUERY_CONSTANTS.DIVISION_QUERY_LIKE;
		}
	...
	public List<LovBean> lovProdCode(Object value){
		...
		try {
			listItemUtil = new ListItemUtil();
			list = listItemUtil.P_CALL_LOV("PILR307_012", "REP_PARAM", "M_FM_PROD_CODE", 
					null, null, null, null, null, (String)value);
		} catch (Exception e) {
			getErrorMap().put(PELConstants.PEL_ERROR_WARNING_CURRENT_KEY,e.getMessage());
			getErrorMap().put("prepareLovCommCode",e.getMessage());
	...
	public List<LovBean> lovPolNo(Object value){
		...
		try {
			listItemUtil = new ListItemUtil();
			list = listItemUtil.P_CALL_LOV("PILR307_012", "REP_PARAM", "M_FM_POL_NO", 
					null, null, null, null, null, (String)value);
		} catch (Exception e) {
			getErrorMap().put(PELConstants.PEL_ERROR_WARNING_CURRENT_KEY,e.getMessage());
			getErrorMap().put("prepareLovCommCode",e.getMessage());
	...
	public List<LovBean> lovGranteeCode1(Object obj){
		...
		if("*".equals(code)){
			object=new Object[]{PELConstants.suggetionRecordSize};
			query=PILR307_012_QUERY_CONSTANTS.CUST_CLASS;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR307_012_QUERY_CONSTANTS.CUST_CLASS_LIKE;
		}
		suggestionList = delegate.prepareLovList(query, object);
		return suggestionList;
	}
	...
	public void validateUI_M_DATE_TO(FacesContext facesContext,
		...
		try {
			...
			if (IP_REP_INFO_BEAN.getUI_M_DATE_TO().before(IP_REP_INFO_BEAN.getUI_M_DATE_FM())){
	...
	public List<LovBean> lovClassCode(Object obj){
		...
		String query=null;
		 if("*".equals(code)){
			object=new Object[]{PELConstants.suggetionRecordSize};
			query=PILR307_012_QUERY_CONSTANTS.CLASS_QUERY ;
		}else{
			object=new Object[]{code+"%",code+"%",PELConstants.suggetionRecordSize};
			query=PILR307_012_QUERY_CONSTANTS.CLASS_QUERY_LIKE;
		}
