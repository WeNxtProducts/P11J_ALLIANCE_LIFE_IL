...
public class PT_IL_POL_MED_EXAM_DTL_1_HELPER {
	...
	public void preInsert(PT_IL_POL_MED_EXAM_DTL_1 medicalDetail,
		...
		try {
				...
				medicalDetail.setPMED_POAD_SYS_ID(assured1Bean.getPOAD_POL_SYS_ID());
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
		} finally {
	...
	public void PMED_CLINIC_CODE_WHEN_VALIDATE_ITEM(
			...
			if (examDetail.getPMED_CLINIC_CODE() != null) {
				...
				if (resultSet.next()) {
					...
					throw new Exception(Messages.getString(PELConstants.pelErrorMessagePath,"3206",
											new Object[] { "This clinic is not in the same area" }));
				}
				CommonUtils.closeCursor(resultSet);
	...
	public void PMED_STATUS_WHEN_VALIDATE_ITEM(
			PT_IL_POL_MED_EXAM_DTL_1_ACTION examAction) throws Exception {
