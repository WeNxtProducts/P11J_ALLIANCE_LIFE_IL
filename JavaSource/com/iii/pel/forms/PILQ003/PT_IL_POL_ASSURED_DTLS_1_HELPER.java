...
public class PT_IL_POL_ASSURED_DTLS_1_HELPER {
	...
	public void preInset(
		...
		PKG_PILQ003 localProcudeure = new PKG_PILQ003();
		...
		try {
			...
			if ("1".equals(DUMMY_OCC_BEAN.getUI_M_CALC_METHOD())) {
				if (PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_CATG_CODE() != null
						&& PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_ADDL_RISK_CODE() != null) {
		...
		try {
			if ("1".equals(CommonUtils.SUBSTR(CommonUtils.getGlobalVariable(
				...
				while (resultSet.next()) {
					...
					fieldName = "B_PAMH_FIELD_" + paddedString;
					method = BP.class
					...
					method = PT_IL_POL_ASSR_MED_HIST.class.getMethod("get"
							+ fieldName);
	...
	public void whenCreateRecord(
			...
			PT_IL_POLICY PT_IL_POLICY_BEAN,PILQ003_COMPOSITE_ACTION compositeAction ) throws Exception {
	...
	public void postQuery(
		...
		try {
			...
			if ("Y".equals(CommonUtils.nvl(PT_IL_POLICY_BEAN.getPOL_FRZ_FLAG(),"X"))) {
				list9 = procedures.callCALC_ASSRD_THRESHOLD_LIMIT(
				...
				if (list9 != null && !list9.isEmpty()) {
					if (list9.get(0) != null) {
						MS_POAD_LC_THRESHOLD_LIMIT = Double.parseDouble(list9.get(0));
					}
				}
	...
	public String postInsert(PT_IL_POLICY PT_IL_POLICY_BEAN, DUMMY DUMMY_OCC_BEAN, 
				PT_IL_POL_ASSURED_DTLS_1 PT_IL_POL_ASSURED_DTLS_1_BEAN)
			throws Exception {
		...
		try {
			...
			/*list = procedures.callP_INS_ADDL_COVER(PT_IL_POLICY_BEAN
					.getPOL_SYS_ID().toString(), PT_IL_POLICY_BEAN
					.getPOL_PROD_CODE());
			if (list != null && !list.isEmpty()) {
				if (list.get(0) != null) {
					DUMMY_OCC_BEAN.setUI_M_CHANGE_SA_YN(list.get(0));
				}
			}
			connection = CommonUtils.getConnection();
	...
	public void POAD_REF_ID1_WHEN_VALIDATE_ITEM(
			PILQ003_COMPOSITE_ACTION compositeAction,PT_IL_POL_ASSURED_DTLS_1_ACTION  assuredAction1) throws Exception {
	...
	public void POAD_HEIGHT_WHEN_VALIDATE_ITEM(Double POAD_HEIGHT,
		...
		try {
			...
			if (POAD_HEIGHT != null && PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_WEIGHT() != null) {
	...
	public void POAD_HEIGHT_UNIT_WHEN_LIST_CHANGED(String POAD_HEIGHT_UNIT,
		...
		try {
			...
			if (PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_HEIGHT() != null
					&& PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_WEIGHT() != null) {
				list = pilt002Procedures.L_BMI_CALC(
	...
	public void POAD_WEIGHT_WHEN_VALIDATE_ITEM(Double POAD_WEIGHT,
		...
		try {
			...
			if (PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_HEIGHT() != null && POAD_WEIGHT != null) {
				list = pilt002Procedures.L_BMI_CALC(
						PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_HEIGHT(),
						POAD_WEIGHT,
						PT_IL_POL_ASSURED_DTLS_1_BEAN.getPOAD_HEIGHT_UNIT(),
	...
	public Double POAD_FC_SUM_ASSURED_VALIDATE_ITEM(Double POAD_FC_SUM_ASSURED,
		...
		try {
			...
			if (list != null && !list.isEmpty()) {
				if (list.get(0) != null) {
					POAD_FC_SUM_ASSURED = Double.parseDouble(list1.get(0));
				}
			}
