...
public class PT_IL_POL_APPL_PROD_ACTION extends CommonAction {
    ...
    private PT_IL_POL_APPL_PROD PT_IL_POL_APPL_PROD_BEAN;

    public PILQ003_COMPOSITE_ACTION compositeAction;

    private PT_IL_POL_APPL_PROD_HELPER helper;
    ...
    private List<PT_IL_POL_APPL_PROD> dataList_PT_IL_POL_APPL_PROD = new ArrayList<PT_IL_POL_APPL_PROD>();

    private UIData dataTable;

    private List<SelectItem> listPAPP_STATUS = new ArrayList<SelectItem>();

    public PT_IL_POL_APPL_PROD_ACTION() {
	Connection connection = null;
	try{
	    	connection = CommonUtils.getConnection();
        	PT_IL_POL_APPL_PROD_BEAN = new PT_IL_POL_APPL_PROD();
        	helper = new PT_IL_POL_APPL_PROD_HELPER();
    ...
    public PT_IL_POL_APPL_PROD getPT_IL_POL_APPL_PROD_BEAN() {
	return PT_IL_POL_APPL_PROD_BEAN;
    }

    public void setPT_IL_POL_APPL_PROD_BEAN(
	    PT_IL_POL_APPL_PROD PT_IL_POL_APPL_PROD_BEAN) {
	this.PT_IL_POL_APPL_PROD_BEAN = PT_IL_POL_APPL_PROD_BEAN;
    }

    public List<PT_IL_POL_APPL_PROD> getDataList_PT_IL_POL_APPL_PROD() {
	return dataList_PT_IL_POL_APPL_PROD;
    }

    public void setDataListPT_IL_POL_APPL_PROD(
	    List<PT_IL_POL_APPL_PROD> dataList_PT_IL_POL_APPL_PROD) {
	this.dataList_PT_IL_POL_APPL_PROD = dataList_PT_IL_POL_APPL_PROD;
    }
    ...
    public void addRow(ActionEvent event) {
    	...
    	try {
    		if (isINSERT_ALLOWED()) {
    			PT_IL_POL_APPL_PROD_BEAN = new PT_IL_POL_APPL_PROD();
    			helper.whenCreateRecord(PT_IL_POL_APPL_PROD_BEAN);
    			resetAllComponent();
    			resetSelectedRow();
    		} else {
     ...
     public void deleteRow(ActionEvent event) {
    	 ...
    	 try {
    		 if (isDELETE_ALLOWED()) {
    		     	helper.onCheckDeleteMaster(PT_IL_POL_APPL_PROD_BEAN);
    		     	helper.keyDelRec(this, compositeAction.getPT_IL_POLICY_ACTION_BEAN().getPT_IL_POLICY_BEAN(),compositeAction);
    		     	new CRUDHandler().executeDelete(PT_IL_POL_APPL_PROD_BEAN,CommonUtils.getConnection());
    		     	message = Messages.getString(PELConstants.pelMessagePropertiesPath,"errorPanel$message$delete");
    		     	dataList_PT_IL_POL_APPL_PROD.remove(PT_IL_POL_APPL_PROD_BEAN);
    		     	if (dataList_PT_IL_POL_APPL_PROD.size() > 0) {
    		     		PT_IL_POL_APPL_PROD_BEAN = dataList_PT_IL_POL_APPL_PROD.get(0);
    		     	} else if (dataList_PT_IL_POL_APPL_PROD.size() == 0) {
    		     		addRow(null);
    		     	}
    		     	resetAllComponent();
   ...
   public void postRecord(ActionEvent event) {
	   String message = null;
	   try {
		   if (PT_IL_POL_APPL_PROD_BEAN.getROWID() == null && isINSERT_ALLOWED()) {
			   helper.preInsert(PT_IL_POL_APPL_PROD_BEAN,compositeAction);
			   new CRUDHandler().executeInsert(PT_IL_POL_APPL_PROD_BEAN,CommonUtils.getConnection());
			   helper.postInsert(PT_IL_POL_APPL_PROD_BEAN);
			   message = Messages.getString(PELConstants.pelMessagePropertiesPath,"errorPanel$message$save"); 
			   dataList_PT_IL_POL_APPL_PROD.add(PT_IL_POL_APPL_PROD_BEAN);
	    } else if (PT_IL_POL_APPL_PROD_BEAN.getROWID() != null&& isUPDATE_ALLOWED()) {
	    	helper.preUpdate(PT_IL_POL_APPL_PROD_BEAN);
	    	new CRUDHandler().executeUpdate(PT_IL_POL_APPL_PROD_BEAN,CommonUtils.getConnection());
	...
	try {
	    resetSelectedRow();
	    PT_IL_POL_APPL_PROD_BEAN = (PT_IL_POL_APPL_PROD) dataTable
		    .getRowData();
	    PT_IL_POL_APPL_PROD_BEAN.setRowSelected(true);
	    resetAllComponent();
	} catch (Exception exc) {
	...
	Iterator<PT_IL_POL_APPL_PROD> PT_IL_POL_APPL_PROD_ITR = dataList_PT_IL_POL_APPL_PROD
		.iterator();
	while (PT_IL_POL_APPL_PROD_ITR.hasNext()) {
	    PT_IL_POL_APPL_PROD_ITR.next().setRowSelected(false);
	}
    }
    ...
    public ArrayList<com.iii.premia.common.bean.LovBean> lovPAPP_PROD_CODE(
	    Object currValue) {
	ArrayList<com.iii.premia.common.bean.LovBean> suggestionList = new ArrayList<com.iii.premia.common.bean.LovBean>();
	ListItemUtil listitemutil = new ListItemUtil();
	try {
	    // P_CALL_LOV('PT_IL_POL_APPL_PROD.PAPP_PROD_CODE',
	    // :PT_IL_POLICY.POL_PROD_GROUP_CODE);
	    suggestionList = listitemutil.P_CALL_LOV("PILT002_APAC",
		    "PT_IL_POL_APPL_PROD", "PAPP_PROD_CODE", compositeAction
			    .getPT_IL_POLICY_ACTION_BEAN()
			    .getPT_IL_POLICY_BEAN().getPOL_PROD_GROUP_CODE(),
	...
	try {
	    PT_IL_POL_APPL_PROD_BEAN.setPAPP_FC_SA((Double) value);
	    helper.PAPP_FC_SA_WHEN_VALIDATE_ITEM(PT_IL_POL_APPL_PROD_BEAN,
		...
		ErrorHelpUtil.getWarningForProcedure(
			CommonUtils.getConnection(), "PAPP_FC_SA", getWarningMap());
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	}
	...
	try {
	    ...
	    PT_IL_POL_APPL_PROD_BEAN.setPAPP_PROD_CODE((String) value);
	    
	    helper.PAPP_PROD_CODE_WHEN_VALIDATE_ITEM(PT_IL_POL_APPL_PROD_BEAN,
		    compositeAction.getPT_IL_POLICY_ACTION_BEAN()
			    .getPT_IL_POLICY_BEAN(), compositeAction
			    .getPT_IL_POL_ASSURED_DTLS_ACTION_BEAN()
			    .getPT_IL_POL_ASSURED_DTLS_BEAN(),componentValue);
	    COMP_PAPP_PROD_CODE.resetValue();
	    COMP_UI_M_PAPP_PROD_DESC.resetValue();
	    COMP_PAPP_PLAN_CODE.resetValue();
	...
	try {
	    PT_IL_POL_APPL_PROD_BEAN.setPAPP_PLAN_CODE((String) value);
	    helper.PAPP_PLAN_CODE_WHEN_VALIDATE_ITEM(PT_IL_POL_APPL_PROD_BEAN);
	    COMP_PAPP_PLAN_CODE.resetValue();
	    COMP_UI_M_PAPP_PLAN_DESC.resetValue();
	} catch (Exception exception) {
	    exception.printStackTrace();
	...
	try {
		...
		helper.whenNewRecordInstance(PT_IL_POL_APPL_PROD_BEAN,
				compositeAction.getPT_IL_POLICY_ACTION_BEAN().getPT_IL_POLICY_BEAN());
		helper.executeQuery(compositeAction);
		helper.on_Populate_Detail(compositeAction);
		if (PT_IL_POL_APPL_PROD_BEAN.getROWID() != null) {
		    
			helper.postQuery(PT_IL_POL_APPL_PROD_BEAN,
				compositeAction.getPT_IL_POLICY_ACTION_BEAN().getPT_IL_POLICY_BEAN());
		}else {
			helper.whenCreateRecord(PT_IL_POL_APPL_PROD_BEAN);
		}
    ...
    /**
     * Disables all components in PT_IL_POL_APPL_PROD_ACTION
     */
    ...
    /**
     * Instantiates all components in PT_IL_POL_APPL_PROD_ACTION
     */
	...
	public String coverGroupDetail(){
		...
		reString = "PILT002_APAC_PT_IL_POL_COVER_GROUP";
	return 	reString;
	}
