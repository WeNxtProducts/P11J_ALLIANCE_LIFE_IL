...
public class COPY_HELPER {
    ...
    public String  UI_M_BUT_COPY_WHEN_BUTTON_PRESSED(
		PILQ003_COMPOSITE_ACTION copmAction) throws Exception {
		...
		ArrayList<OracleParameter> list =new P9ILPK_POLICY().COPY_POLICY(
			CommonUtils.getProcedureValue(
				...
				CommonUtils.getProcedureValue(	copyBean.getUI_M_COPY_PT_IL_POL_ASS_DTL()), 
                        ...
                        CommonUtils.getProcedureValue(policyBean.getPOL_UW_YEAR()),
				CommonUtils.getDateAsStringForOracleProcedureCall(copyBean
						.getUI_M_COPY_POL_ISSUE_DT()), "C", M_NEW_POL_SYS_ID,CommonUtils.getProcedureValue(	copyBean.getUI_M_COPY_PT_IL_POL_ADDL_COVER()));
	...
	*/
    ...
    public void  UI_M_COPY_POL_ISSUE_DT_WHEN_VALIDATE_ITEM( Date UI_M_COPY_POL_ISSUE_DT,
	    COPY_ACTION copyAction,PT_IL_POLICY policyBean	) throws Exception {
	CommonUtils commonUtils = new CommonUtils();
	...
	copyBean.setUI_M_COPY_POL_ISSUE_DT(UI_M_COPY_POL_ISSUE_DT);
	try {
		...
		if (copyBean.getUI_M_COPY_POL_ISSUE_DT() != null) {
			...
			if (vembu_C1.next()) {
				...
				if ("N".equals(copyBean.getUI_M_POL_NEW_YN())) {
				 ...
				 pvalList =  new PKG_PILT002_APAC().P_SET_DOC_NO_PROP(policyBean.getPOL_DS_TYPE(),
						...
						commonUtils.getProcedureValue(copyBean.getUI_M_COPY_UW_YEAR()),
	                		      "COPY.M_COPY_POL_NO",
	                		      "COPY",
	                		      "Y",commonUtils.dateToStringFormatter(  policyBean
						.getPOL_ISSUE_DT()),
						null,null,policyBean.getROWID(),P_DISABLE_YN);
				 if(!pvalList.isEmpty()){
				     copyBean.setUI_M_COPY_POL_NO((String)pvalList.get(0).getValueObject());
				     P_DISABLE_YN =(String) pvalList.get(1).getValueObject();
				 }
