...
public interface PILR384_012_QUERY_CONSTANTS {
	...
	String DIVISION_QUERY=
		" SELECT  DIVN_CODE,DIVN_NAME FROM MV_DIVISION WHERE  ROWNUM<=? ORDER BY 1 ";
	
	String DIVISION_QUERY_LIKE=
		" SELECT  DIVN_CODE,DIVN_NAME FROM MV_DIVISION WHERE " +
		" (DIVN_CODE LIKE ?  OR DIVN_NAME LIKE ?) AND ROWNUM<=? ORDER BY 1 ";
	
	String DEPARTMENT_QUERY=
		" SELECT DEPT_CODE,DEPT_NAME FROM PM_DEPARTMENT WHERE  " +
		" DEPT_FRZ_FLAG = 'N' AND ROWNUM<=? ORDER BY 1 ";
	
	String DEPARTMENT_QUERY_LIKE=
		" SELECT DEPT_CODE,DEPT_NAME FROM PM_DEPARTMENT WHERE  " +
		" DEPT_FRZ_FLAG = 'N' AND (DEPT_CODE LIKE ?  OR DEPT_NAME LIKE ? ) AND ROWNUM<=? ORDER BY 1 ";
	...
	String POL_LIKE = "SELECT POL_NO FROM PT_IL_POLICY WHERE POL_NO LIKE ? AND ROWNUM<?";
}
